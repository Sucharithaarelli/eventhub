...
Input: `docs/test-scenarios.md` (60 scenarios, TC-001 to TC-511)
...
| **API / Integration** | 28 | Backend contract, auth enforcement, business rule execution, DB state, validator rules | 10–30s total |
| **Component** | 15 | Client-side UI states, conditional rendering, mocked API responses | 5–15s total |
| **E2E** | 12 | Critical user journeys, multi-page flows, cross-session security | 2–5 min total |
| **Total** | **60** | | |

**Pyramid shape**: Unit (5) → API (28) → Component (15) → E2E (12) ✓
...
| TC-309 | Invalid phone format (letters/symbols) returns 400 | `bookingValidator.validateCreateBooking` | `.matches(/^[0-9+\-\s()]+$/)` at `bookingValidator.js:36` |
| TC-310 | customerName with 1 character returns 400 | `bookingValidator.validateCreateBooking` | `.isLength({ min: 2 })` at `bookingValidator.js:24` |
| TC-311 | eventId = 0 or negative returns 400 | `bookingValidator.validateCreateBooking` | `.isInt({ min: 1 })` at `bookingValidator.js:17` |
| TC-312 | Invalid customerEmail format returns 400 | `bookingValidator.validateCreateBooking` | `.isEmail()` at `bookingValidator.js:30` |

#### Edge Cases — API Filters
| TC | Title | Endpoint | Repository Filter |
|---|---|---|---|
| TC-409 | Filter bookings by `?eventId` returns only that event's bookings | `GET /api/bookings?eventId=3` | `bookingRepository.findAll` — `where.eventId` applied at `bookingRepository.js:11` |
| TC-410 | Filter bookings by `?status=confirmed` returns only confirmed bookings | `GET /api/bookings?status=confirmed` | `bookingRepository.findAll` — `where.status` applied at `bookingRepository.js:12`; all active bookings are always `confirmed` |
...
#### Sandbox Banner — `BookingsContent` component (`page.tsx:14`)
| TC | Title | Mocked State | Assertion |
|---|---|---|---|
| TC-511 | Sandbox warning banner appears near booking limit | Mock `GET /api/bookings` returning 8 or 9 bookings | Banner text contains "sandbox holds up to 6 events and 9 bookings"; absent when booking count < 5 |
...
| Testing phone/email/name field validation by filling the booking form at E2E | TC-309, TC-310, TC-312 | API test: `POST /api/bookings` with bad values — proves `bookingValidator.js` rules without browser overhead |
...
- `TC-304, TC-305, TC-306, TC-309, TC-310, TC-311, TC-312` (validation — API)
...
- `TC-107, TC-407, TC-409, TC-410` (pagination + filters — API)
- `TC-402, TC-403` (quantity UI boundaries — E2E)
- `TC-507, TC-508, TC-510, TC-511` (UI micro-states + sandbox banner — Component)
